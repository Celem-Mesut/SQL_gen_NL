...
from lineage import build_lineage_dot, build_lineage_mermaid, find_terminal_views
from llm_client import DEFAULT_MODEL
from ui_helpers import (
    _stage_icon,
    cached_build_lineage_index,
    cached_generate_all_views,
    render_stage,
)
            ...
            if multi_stage:
                ...
                for stage_name, stage_df in stages.items():
                    if stage_name not in st.session_state.output_all_sqls:
                        results, _ = cached_generate_all_views(stage_df, use_create_or_alter, add_go)
...
# ============================================================================
    ...
    lineage_index = cached_build_lineage_index(st.session_state.stages)
...
# ============================================================================
    ...
    use_create_or_alter = st.session_state.get("opt_create_or_alter", True)
    add_go = st.session_state.get("opt_add_go", True)
    lineage_index = cached_build_lineage_index(stages)
    ...
    results, _warnings = cached_generate_all_views(stage_df, use_create_or_alter, add_go)
    ...
    for b_stage_name, b_stage_df in stages.items():
        b_results, _ = cached_generate_all_views(b_stage_df, use_create_or_alter, add_go)
